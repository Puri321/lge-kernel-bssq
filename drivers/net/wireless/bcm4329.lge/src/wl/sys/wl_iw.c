...
// joon for test			err = wl_iw_vndr_ie(dev,VNDR_IE_PRBREQ_FLAG|VNDR_IE_ASSOCREQ_FLAG,
		...
		WL_TRACE(("WPS:  ADD_IE : %d, Len : %d , OUI :"
				"%02x:%02x:%02x\n", ie_id, ie_len, ie_buf[pos],
				ie_buf[pos+1], ie_buf[pos+2]));
//joon for test		err = wl_iw_vndr_ie(dev,VNDR_IE_PRBREQ_FLAG | VNDR_IE_ASSOCREQ_FLAG ,ie_buf+pos, VNDR_SPEC_ELEMENT_ID, ie_buf+pos+3,ie_len-3, delete);
