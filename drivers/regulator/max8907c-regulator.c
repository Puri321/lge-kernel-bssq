...
//	printk("[WLED] ##################temp_cpu==%d############################\n",temp_cpu);
		if(temp_cpu > 550) // High temp exception 
		{
			max8907c_reg_write(info->i2c, info->reg_base+1, 90);
					schedule_timeout_interruptible(msecs_to_jiffies(50));

			max8907c_reg_write(info->i2c, info->reg_base+1, 80);
					schedule_timeout_interruptible(msecs_to_jiffies(300));

			max8907c_reg_write(info->i2c, info->reg_base+1, 60);
					schedule_timeout_interruptible(msecs_to_jiffies(300));

			max8907c_reg_write(info->i2c, info->reg_base+1, 40);
					schedule_timeout_interruptible(msecs_to_jiffies(300));

			max8907c_reg_write(info->i2c, info->reg_base+1, 20);
