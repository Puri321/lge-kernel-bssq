...
#if defined (CONFIG_LU6500)
static int dc_color_palette[256] = 
{
	0x0,
	0x10101,
	0x20202,
...
#elif defined (CONFIG_KU8800) || defined (CONFIG_SU880)
static int dc_color_palette[256] = 
{
	0x0,
	0x10101,
...
#if defined (CONFIG_LU6500) || defined (CONFIG_KU8800) || defined (CONFIG_SU880)
static void tegra_dc_init_fb_lut_defaults(struct tegra_dc_lut *lut, int *color_palette)
{
	int i;
	...
	{	
		for (i = 0; i < 256; i++)
		{
			lut->r[i] = dc_color_palette[i] & 0xFF;
			lut->g[i] = (dc_color_palette[i] >> 8) & 0xFF;
			lut->b[i] = (dc_color_palette[i] >> 16) & 0xFF;
		}
		printk("[wonhee.jeong] set lut defaults\n");
	}
}
#endif
...
#if defined (CONFIG_LU6500) || defined (CONFIG_KU8800) || defined (CONFIG_SU880)
static int tegra_dc_update_winlut_internal(struct tegra_dc *dc, int win_idx, int fbovr)
{
...
#if defined (CONFIG_LU6500) || defined (CONFIG_KU8800) || defined (CONFIG_SU880) //WAR for pll setting
...
static void tegra_dc_init(struct tegra_dc *dc)
{
	...
	for (i = 0; i < DC_N_WINDOWS; i++) {
		...
		// set color palette
		if(dc->ndev->id == 0)
			tegra_dc_update_winlut_internal(dc, i, 0);
		else
			tegra_dc_set_lut(dc, win);
#else
		tegra_dc_set_lut(dc, win);
...
/* WAR: Add sleep for Dsi panel especially for LGD dis panel.
 * Some panel should not stop dc stream in the middle of frame.
 * It seems lcd go into unstablle state. */
...
void tegra_dc_disable(struct tegra_dc *dc)
{
	...
	if ((dc->out->type == TEGRA_DC_OUT_DSI || 
		...
		dc->out->postsuspend();
/* WAR : delay is duplicate with mdelay(100) in bssq_dis_panel_postsuspend */
/* delay is duplicate with mdelay(50) in star_dis_panel_postsuspend */
/*		msleep(100); */
	}
#endif
...
#if defined (CONFIG_LU6500) || defined (CONFIG_KU8800) || defined (CONFIG_SU880)
	if(dc->ndev->id == 0)
		tegra_dc_init_fb_lut_defaults(&dc->fb_lut, dc_color_palette);
	else
