...
static struct wake_lock bd_wake_lock;  //                                                 
...
//	dbg(" brightness=%d", bd->props.brightness);
	...
	if (set_auto_brightness > 0) {
		if ((set_alc_option == -1) || (set_alc_option != old_auto_brightness)) { //                                                                         
...
//                                              
				...
				bd6084_bl_write(bd, 0x0B, 0x04);   // ALC slope curve setting -> init value is '0x00'  log curve, 1mA step
				bd6084_bl_write(bd, 0x0D, 0x0E);   // Main Group Min Current  -> init value is '0x0E'  3mA
...
#endif
#elif defined (CONFIG_LU6500)
				bd6084_bl_write(bd, 0x0B, 0x02); // ALC slope curve setting ->  log curve, 1.1mA step
				bd6084_bl_write(bd, 0x0D, 0x0E); // Main Group Min Current = 2.6 mA
				bd6084_bl_write(bd, 0x0E, 0x28); // Main Group Max Current = 7 mA
#endif
#endif
//                                              
			}
			    ...
			    const struct i2c_device_id *id)
{
	...
	wake_lock_init(&bd_wake_lock, WAKE_LOCK_SUSPEND, "bd_wake_lock");  //                                                 
...
static void bd6084_bl_early_suspend(struct early_suspend *h)
{
	...
	wake_lock_timeout(&bd_wake_lock, 5 * HZ);  //                                                 
}
...
{
	wake_lock_destroy(&bd_wake_lock);  //                                                 
