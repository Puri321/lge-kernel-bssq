			...
			int bytes, const void *src)
{
	...
	for (i = 0; i < 5; i++) {
		ret = wm8994->write_dev(wm8994, reg, bytes, src);
		if (ret == 0)
			break;
	}
...
//                                             
	...
	ret = wm8994_read(wm8994, WM8994_GPIO_1, WM8994_NUM_GPIO_REGS * 2,
			  &wm8994->gpio_regs);
	mutex_unlock(&wm8994->io_lock);
	if ((be16_to_cpu(wm8994->gpio_regs[2])!=0x8100)||(be16_to_cpu(wm8994->gpio_regs[0])!=0x0001)){/* Restore the registers */ 
		printk("wm8994 suspend restore error GPIO_3=0x%x\n", be16_to_cpu(wm8994->gpio_regs[2]));

		wm8994->gpio_regs[0]= cpu_to_be16(0x0001); 			 
		wm8994->gpio_regs[2]= cpu_to_be16(0x8100);
		wm8994->gpio_regs[3]= cpu_to_be16(0x8100);
		wm8994->gpio_regs[4]= cpu_to_be16(0x8100);
		wm8994->gpio_regs[6]= cpu_to_be16(0x0100);
	...
	ret = wm8994_read(wm8994, WM8994_LDO_1, WM8994_NUM_LDO_REGS * 2,
						   &wm8994->ldo_regs);
	mutex_unlock(&wm8994->io_lock);
	if ((be16_to_cpu(wm8994->ldo_regs[0])&0x1)||(be16_to_cpu(wm8994->ldo_regs[1])&0x1)){
		printk("LDO registers: wm8994->ldo_regs[0] = 0x%x\n", be16_to_cpu(wm8994->ldo_regs[0]));

		wm8994->ldo_regs[0]= cpu_to_be16(be16_to_cpu(wm8994->ldo_regs[0])&~(0x1));
		wm8994->ldo_regs[1]= cpu_to_be16(be16_to_cpu(wm8994->ldo_regs[1])&~(0x1));
		printk("LDO registers: wm8994->ldo_regs[0] = 0x%x\n", be16_to_cpu(wm8994->ldo_regs[0]));
	}
