...
#elif defined (CONFIG_LU6500) || defined (CONFIG_SU880) || defined (CONFIG_KU8800)
...
static s32 __devinit muic_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
  ...
  if(get_lge_pcb_revision() > REV_D)
	 ...
	 */
#if defined (CONFIG_MACH_BSSQ)
	ret = request_irq(gpio_to_irq(muic_intr_gpio), (irq_handler_t)muic_interrupt_handler,IRQF_TRIGGER_FALLING|IRQF_TRIGGER_RISING, client->name, &client->dev);
#else
	ret = request_irq(gpio_to_irq(MUIC_GPIO), (irq_handler_t)muic_interrupt_handler, IRQF_TRIGGER_FALLING , client->name, &client->dev);
#endif
	if (ret < 0) {
...
{
#ifdef CONFIG_MACH_STAR_SU660 //from x3
