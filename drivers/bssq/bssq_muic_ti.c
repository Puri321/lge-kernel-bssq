...
#ifdef CONFIG_HAS_WAKELOCK
#if defined(CONFIG_LU6500) || defined (CONFIG_SU880) || defined (CONFIG_KU8800)
  wake_lock_timeout(&muic_dev->wlock_timeout,  msecs_to_jiffies(3000));
#else
...
#if defined(CONFIG_SU880) || defined(CONFIG_KU8800)
#define GPIO_IFX_RESET	TEGRA_GPIO_PV0

void muic_ifx_reset()
{
	gpio_request(GPIO_IFX_RESET, "muic_ifx_reset");
	tegra_gpio_enable(GPIO_IFX_RESET);
	gpio_direction_output(GPIO_IFX_RESET, 0);

	gpio_set_value(GPIO_IFX_RESET, 0);
	msleep(100);
	gpio_set_value(GPIO_IFX_RESET, 1);
}
#endif
...
#elif defined(CONFIG_SU880) || defined(CONFIG_KU8800)
	if(mode == PORT_SETTING_AP_USB) {
		printk("[MUIC] AP_USB Mode: Wake Locks\n");
		wake_lock(&muic_dev->wlock);
	}
#if defined(CONFIG_SU880) || defined(CONFIG_KU8800)
	if(mode == PORT_SETTING_CP_DOWNLOAD)
	{
		printk("[MUIC] CP_D/L Mode: Modem Reset\n");
		muic_ifx_reset();
		mode = PORT_SETTING_CP_USB;
	}
#endif
#endif
