...
void charger_ic_disable(void)
{
	...
	p_rt9524->status = CHARGER_DISABLE;

	printk(KERN_DEBUG "[CHARGER] %s : rt9524_status = CHARGER_DISABLE\n",__func__);
}
...
void charger_ic_set_mode(charger_ic_status mode)
{
	...
	printk(KERN_DEBUG "[CHARGER] %s : rt9524_status = %s\n",__func__, charger_ic_name[mode]);
}
...
{
	if (p_rt9524->status != CHARGER_DISABLE) {
