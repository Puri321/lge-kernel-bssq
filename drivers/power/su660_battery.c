...
static void battery_update(struct battery_info *batt_info)
{
	...
	if (( at_boot_state == 1 ) || ( at_charge_index == 1 ) || ( ELT_test_mode == 1 )|| batt_info->temp_control == TEMP_CONTROL_ON)
	{
		batt_info->temperature = 270; 
		if(batt_info->temp_control == TEMP_CONTROL_ON)
		{
			DBG("[TEMP CONTROL]Unlimited Temp Charging ON~~~~~~~~~~~~~~~~", 0);
		}
	}
	...
	{
		DBG("[TEMP CONTROL]Unlimited Temp Charging OFF~~~~~~~~~~~~~~~~", 0);
	}	
...
static void battery_update_changes(struct battery_info *batt_info, int force_update)
{
	...
	DBG("\n[BATT] voltage [%d|%d], Temperature [%d], Capacity/Gauge [%d/%d], Present [%d]\n"
			"[CHARGER] Type [%d], Status [%d]\n", 
			batt_info->voltage, 0/*max17043_get_voltage()*/, batt_info->temperature, batt_info->capacity, batt_info->capacity_gauge, batt_info->present,
	...
	switch (psp) {
		case POWER_SUPPLY_PROP_STATUS:
					...
					|| (batt_info->health == POWER_SUPPLY_HEALTH_COLD))
			{
				...
				//			DBG("STATUS_NOT_CHARGING(%d)", val->intval);
			}
		...
		size_t count)
{
	...
	if (value != 999)
	{
		...
		DBG("Cap [%d] / Val [%d]", batt_info->capacity, value);
	...
	{
		if  (batt_info->charge_status == POWER_SUPPLY_HEALTH_GOOD)
		{
			charger_ic_set_state(CHARGER_STATE_CHARGE);
			charger_ic_set_mode(batt_info->charge_setting_chcomp);
			batt_info->boot_TA_setting = 0;
		}
		else if (batt_info->charge_status == POWER_SUPPLY_HEALTH_OVERHEAT)
		{
			charger_ic_set_state(CHARGER_STATE_CHARGE);
			charger_ic_set_mode(CHARGER_USB500);	
			batt_info->boot_TA_setting = 0;
...
static void valid_cbc_check_and_process(unsigned int value)  // not used  yet 
{
	...
	//	if( at_charge_index == 0 )
	{
		...
		if ( batt_info->gauge_on == 1 )
		{
			...
			else // ( 0 < cbc_value < 100 )
			{
				...
				if( display_cbc >= batt_info->capacity ) 
				{
					...
					{	
						...
						if(g_is_suspend)
						{
							printk("[BATTERY] LCD OFF====>ON Gauge All Update [- batt_info->capacity) > 1]~~~~~~~~~~~~~~~\n");
							batt_info->capacity=display_cbc;
							g_is_suspend=0;
						}
						else
				...
				else if( display_cbc < batt_info->capacity ) 
				{
					...
					else if ((batt_info->capacity - display_cbc) > 1)
					{
						...
						if(g_is_suspend)
						{
							printk("[BATTERY] LCD OFF====>ON Gauge All Update [- display_cbc) > 1]~~~~~~~~~~~~~~~\n");
							batt_info->capacity=display_cbc;
							g_is_suspend=0;
						}
						else
            ...
            printk("[BATTERY] #############valid_cbc_check_and_process() batt_info->capacity[%d]##################\n",batt_info->capacity);
			//battery_update(batt_info);
			power_supply_changed(&batt_info->bat);
		}
	}
...
//                                                          
	...
	queue_delayed_work(batt_info->battery_workqueue, &batt_info->star_id_monitor_work, HZ*40);
//                                                          
	return 0;
}
