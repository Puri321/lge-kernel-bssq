...
struct spi_tegra_data {
	...
	struct workqueue_struct *timer_clk_wq;
	struct work_struct timer_clk_work;
	struct timer_list timer_clk;
	spinlock_t timer_clk_lock;
	bool timer_clk_mode;
	bool timer_is_clkon;
	bool timer_is_readyclkoff;
	int timer_clk_enable_duration;
};
...
unsigned long transfer_sec, transfer_min;
unsigned long transfer_start_sec, transfer_start_min;

void get_kernel_time(unsigned long *sec, unsigned long *min)
{
	unsigned long long t;
	unsigned long nanosec_rem;
	static volatile unsigned int printk_cpu;

	printk_cpu = smp_processor_id();t = cpu_clock(printk_cpu);
	nanosec_rem = do_div(t, 1000000000);
	*sec = (unsigned long)t;
	*min = (nanosec_rem / 1000);
}
...
static void spi_tegra_timer_clk_work(struct work_struct *timer_clk_work)
{
	unsigned long flags;
	struct spi_tegra_data *tspi = container_of(timer_clk_work,
				struct spi_tegra_data, timer_clk_work);

	spin_lock_irqsave(&tspi->timer_clk_lock, flags);
	if (tspi->timer_is_clkon && tspi->timer_is_readyclkoff) {
		tspi->timer_is_clkon = false;
		spin_unlock_irqrestore(&tspi->timer_clk_lock, flags);

		tegra_spi_clk_disable(tspi);
		pm_runtime_put_sync(&tspi->pdev->dev);

		printk("##@%s clk disabled, st(%d) bus_num(%d) \n", __func__, 
			tspi->clk_state, tspi->master->bus_num);
	} else {
		spin_unlock_irqrestore(&tspi->timer_clk_lock, flags);
		printk("##@%s skipped, is_clkon(%d), is_readyclkoff(%d), st(%d) bus_num(%d) \n",
				__func__, tspi->timer_is_clkon,
				tspi->timer_is_readyclkoff, tspi->clk_state, tspi->master->bus_num);
	}
}

void timer_clk_func(unsigned long data)
{
	struct spi_tegra_data *tspi = (struct spi_tegra_data *)data;
	unsigned long flags;

	spin_lock_irqsave(&tspi->timer_clk_lock, flags);
	if (tspi->timer_is_readyclkoff && tspi->timer_is_clkon
		 && !timer_pending(&tspi->timer_clk)) {
		spin_unlock_irqrestore(&tspi->timer_clk_lock, flags);
		queue_work(tspi->timer_clk_wq, &tspi->timer_clk_work);
		printk("##@%s timer expired & queue work, st(%d) bus_num(%d) \n", __func__,
			tspi->clk_state, tspi->master->bus_num);
	} else {
		spin_unlock_irqrestore(&tspi->timer_clk_lock, flags);
		printk("##@%s pending timer or is_clkon(%d), st(%d), readyclkoff(%d) bus_num(%d) \n",
			__func__, tspi->timer_is_clkon, tspi->clk_state,
			tspi->timer_is_readyclkoff, tspi->master->bus_num);
	}
}

static void spi_tegra_timer_clk_enable(struct spi_tegra_data *tspi)
{
	unsigned long flags;
	int ret;

	spin_lock_irqsave(&tspi->timer_clk_lock, flags);
	if (timer_pending(&tspi->timer_clk)) {
			del_timer(&tspi->timer_clk);
	}
	tspi->timer_is_readyclkoff = false;
	if (!tspi->timer_is_clkon) {
		tspi->timer_is_clkon = true;
		spin_unlock_irqrestore(&tspi->timer_clk_lock, flags);

		pm_runtime_get_sync(&tspi->pdev->dev);
		tegra_spi_clk_enable(tspi);
		printk("##@%s clk enabled (%d) bus_num(%d) \n", __func__, tspi->clk_state, tspi->master->bus_num);
	} else 
		spin_unlock_irqrestore(&tspi->timer_clk_lock, flags);
}


static void spi_tegra_timer_clk_disable(struct spi_tegra_data *tspi)
{
	unsigned long flags;
	int ret;
	...
	spin_lock_irqsave(&tspi->timer_clk_lock, flags);
	if (tspi->timer_is_clkon) {
		tspi->timer_is_readyclkoff = true;
		ret = mod_timer(&tspi->timer_clk, jiffies +
			msecs_to_jiffies(tspi->timer_clk_enable_duration));
		spin_unlock_irqrestore(&tspi->timer_clk_lock, flags);
	} else {
		spin_unlock_irqrestore(&tspi->timer_clk_lock, flags);
		dev_err(&tspi->pdev->dev, "clk err! should be on\n");
	}
}
		    ...
		    bool is_single_xfer)
{
	...
	if (is_tx_timeout)
		printk("##@%s clkon(%d), clkstate(%d), prev call[%5lu.%06lu] bus_num(%d) \n", __func__, 
				tspi->timer_is_clkon, tspi->clk_state,
				transfer_start_sec, transfer_start_min, tspi->master->bus_num) ;

	get_kernel_time(&transfer_start_sec, &transfer_start_min);

	if (is_first_of_msg) {
		if (tspi->timer_clk_mode)
			spi_tegra_timer_clk_enable(tspi);
		else {
			pm_runtime_get_sync(&tspi->pdev->dev);
			tegra_spi_clk_enable(tspi);
		}
		...
		printk("##@%s not first of msg is_clkon(%d) bus_num(%d) \n", __func__,
				tspi->timer_is_clkon, tspi->master->bus_num) ;
...
static int spi_tegra_transfer(struct spi_device *spi, struct spi_message *m)
{
	...
	if (is_tx_timeout)
		printk("##@%s, prev call[%5lu.%06lu] bus_num(%d) \n", __func__,
			transfer_sec, transfer_min, tspi->master->bus_num) ;
	
	get_kernel_time(&transfer_sec, &transfer_min);
	...
	unsigned err, unsigned cur_xfer_size, unsigned long *irq_flags)
{
	...
	if (!list_is_last(&tspi->cur->transfer_list, &m->transfers)) {
		...
		if (!list_empty(&tspi->queue)) {
			...
			udelay(10);
			if (tspi->timer_clk_mode) {
				spi_tegra_timer_clk_disable(tspi);
			} else {
				tegra_spi_clk_disable(tspi);
				pm_runtime_put_sync(&tspi->pdev->dev);
			}
...
static int __init spi_tegra_probe(struct platform_device *pdev)
{
	...
	if (pdata) {
		...
		tspi->timer_clk_mode = true;
		tspi->timer_clk_enable_duration = 2000;
	} else {
	...
	printk("##@%s : bus_num(%d) is_clkon_always(%d) timer_clk_mode(%d)\n", __func__, tspi->master->bus_num, tspi->is_clkon_always, tspi->timer_clk_mode);
    //                                                                                             
...
skip_dma_alloc:
	...
	if (tspi->timer_clk_mode) {
		tspi->timer_clk_wq =
			create_singlethread_workqueue("spi_tegra_timer_wq");
		if (!tspi->timer_clk_wq) {
			dev_err(&pdev->dev, "can not create workqueue "
					"timer clk mode off\n");
			tspi->timer_clk_mode = false;
		} else {
			spin_lock_init(&tspi->timer_clk_lock);
			init_timer(&tspi->timer_clk);
			tspi->timer_clk.function = timer_clk_func;
			tspi->timer_clk.data = (unsigned long)tspi;
			INIT_WORK(&tspi->timer_clk_work,
					spi_tegra_timer_clk_work);
		}
	}
...
static int __devexit spi_tegra_remove(struct platform_device *pdev)
{
	...
	if (tspi->timer_clk_mode) {
		del_timer_sync(&tspi->timer_clk);
		cancel_work_sync(&tspi->timer_clk_work);
	}

	destroy_workqueue(tspi->spi_workqueue);
	destroy_workqueue(tspi->timer_clk_wq);

	if (tspi->timer_clk_mode && tspi->timer_is_clkon) {
		tegra_spi_clk_disable(tspi);
		tspi->timer_is_clkon = false;
	}
