...
{
	if (!das) {
		pr_err("%s(%d): Das Device Struct not Exist.\n"
			, __FUNCTION__, __LINE__);
		return -ENODEV;
	}

        mutex_lock(&das->mlock);
        if (normal) {
                /* Enable the DAP outputs */
                if (das->tristate_count == 0) {
        ...
        mutex_unlock(&das->mlock);

        return 0;
}

int tegra20_das_power_mode(bool is_normal)
{
	das_set_pin_state(is_normal);
	return 0;
}
EXPORT_SYMBOL_GPL(tegra20_das_power_mode);
#endif
//                                                                 
